/**
 * Rev P — Android TV boot-path, default-display, and Arabic font selector DOM smoke test.
 *
...
(async () => {
  ...
  check("Rev P retains TV settings migration marker saved", savedSettings.tvVisualRevision === "J-tv-closure-premium-leanback-architecture");
  check("Clean Naskh is the default Arabic font", savedSettings.arabicFont === "noto-naskh", `arabicFont=${savedSettings.arabicFont}`);
  const fontMap = (window.__azkar && window.__azkar.AR_FONTS) || {};
  check("Rev P exposes four Arabic font choices", ["noto-naskh", "scheherazade", "amiri", "noto-sans-arabic"].every((k) => !!fontMap[k]));
  const arabicFontVar = window.document.documentElement.style.getPropertyValue("--arabic-font-family");
  check("Default Arabic font CSS variable is applied", arabicFontVar.includes("Noto Naskh Arabic"));
  ...
  check("Item name/title defaults ON on TV", savedSettings.showTitle === true);
  ...
  check("Item name/title visible on first TV render", title && !title.classList.contains("hidden") && title.textContent.trim().length > 0);
  ...
  let arabicFontRow = false;
  if (settingsBody) settingsBody.querySelectorAll(".tv-set-name").forEach((n) => { if (/arabic font/i.test(n.textContent)) arabicFontRow = true; });
  check("Arabic Font row exists", arabicFontRow);

  const fontSelect = window.document.querySelector(".tv-drop-select[data-tv-drop-select=\"arabicFont\"]");
  check("TV Arabic Font dropdown button exists", !!fontSelect);
  if (fontSelect) {
    fontSelect.click();
    await wait(40);
    const fontMenu = window.document.querySelector(".tv-drop-menu.open");
    const fontOptions = Array.from(window.document.querySelectorAll(".tv-drop-option"));
    check("TV Arabic Font dropdown opens", !!fontMenu);
    check("TV Arabic Font dropdown has four options", fontOptions.length === 4);
    const amiriOption = fontOptions.find((opt) => JSON.parse(opt.getAttribute("data-drop-val")) === "amiri");
    check("Classic Arabic option is present", !!amiriOption);
    if (amiriOption) {
      amiriOption.click();
      await wait(60);
      const savedAfterFont = JSON.parse(window.localStorage.getItem("azkartv.v02.settings") || "{}");
      check("TV Arabic Font selection saves", savedAfterFont.arabicFont === "amiri");
      check("Separate Arabic font key saves", window.localStorage.getItem("duaZikrArabicFont") === "amiri");
      check("Selected Arabic font CSS variable is applied", window.document.documentElement.style.getPropertyValue("--arabic-font-family").includes("Amiri"));
    }
  }
  ...
  console.log("\n===== Rev P DOM SMOKE TEST (Android TV mode) =====");
