/* ==========================================================================
   Dua & Zikr — Rev P Arabic Sentence Ending Lock
...
(function () {
  ...
  var DEFAULTS = {
    ...
    arabicFont: "noto-naskh",         // noto-naskh | scheherazade | amiri | noto-sans-arabic
    ...
    showTitle: true,                   // item name/title visible by default; user can hide from Settings
    showPauseMarks: true,
    ...
    tvVisualRevision: "J-tv-closure-premium-leanback-architecture"              // TV-only visual settings migration marker
  };
  ...
  var AR_FONT_LABELS = {
    "noto-naskh": "Clean Naskh",
    "scheherazade": "Traditional Naskh",
    "amiri": "Classic Arabic",
    "noto-sans-arabic": "Simple Arabic"
  };
  var AR_FONT_OPTIONS = [
    ["Clean Naskh", "noto-naskh"],
    ["Traditional Naskh", "scheherazade"],
    ["Classic Arabic", "amiri"],
    ["Simple Arabic", "noto-sans-arabic"]
  ];

  function normalizeArabicFont(value) {
    value = String(value || "").trim();
    if (value === "naskh" || value === "noto" || value === "noto-naskh-arabic") return "noto-naskh";
    if (value === "scheherazade-new" || value === "traditional-naskh") return "scheherazade";
    if (value === "noto-sans" || value === "simple-arabic") return "noto-sans-arabic";
    if (Object.prototype.hasOwnProperty.call(AR_FONTS, value)) return value;
    return DEFAULTS.arabicFont;
  }

  function arabicFontFamily(value) {
    value = normalizeArabicFont(value);
    return AR_FONTS[value] || AR_FONTS[DEFAULTS.arabicFont];
  }

  function arabicFontLabel(value) {
    value = normalizeArabicFont(value);
    return AR_FONT_LABELS[value] || AR_FONT_LABELS[DEFAULTS.arabicFont];
  }

  function applyArabicFontSelection(model) {
    var font = normalizeArabicFont(model && model.arabicFont);
    document.documentElement.setAttribute("data-arabic-font", font);
    document.documentElement.style.setProperty("--arabic-font-family", arabicFontFamily(font));
    return font;
  }
  ...
  function normalizeSettings(input) {
    ...
    num("arScale", 0.1, 2.0, DEFAULTS.arScale);
    ...
    ["showArabic", "showTranslit", "showEnglish", "showUrdu", "showTranslation", "showSource", "showTitle", "showPauseMarks", "showWaqfLegend", "showRibbon", "tajweed", "showCopy", "showShare", "tvFit", "easyView", "autoRotate", "smartSentenceFlow", "showProgress", "swipeNav", "showTags"].forEach(function (k) { s[k] = !!s[k]; });
    ...
    s.arabicFont = normalizeArabicFont(s.arabicFont);
  ...
  var I18N = {
    en: {
      ...
      theme:"Theme", themeDesc:"Choose Dark or Light appearance. Live preview applies immediately.",
      ...
      arabicTypeface:"Arabic script", arabicTypefaceDesc:"Arabic script style for the reader. Live preview on change.",
      arabicFont:"Arabic Font", arabicFontDesc:"Choose the Arabic reading font. Clean Naskh is the default.",
      ...
      contentVersion:"Content version", lastUpdated:"Last updated", canonicalItems:"Canonical items", sectionReferences:"Section references", fontNote:"Arabic font is user-selectable; Urdu uses Noto Nastaliq Urdu.", reviewBadge:"Content review status: pending scholarly review",
      ...
      fontCleanNaskh:"Clean Naskh", fontTraditionalNaskh:"Traditional Naskh", fontClassicArabic:"Classic Arabic", fontSimpleArabic:"Simple Arabic",
      theme_elder_dark:"Elder Ease Dark", theme_elder_light:"Elder Ease Light", theme_dark_ambient:"Dark Ambient", theme_gold_navy:"Gold & Navy", theme_haram_light:"Haram Light", theme_green_classic:"Green Classic", theme_sepia:"Sepia",
    ...
    ur: {
      ...
      theme:"تھیم", themeDesc:"ڈارک یا لائٹ انداز منتخب کریں۔",
      ...
      contentVersion:"مواد ورژن", lastUpdated:"آخری اپ ڈیٹ", canonicalItems:"اصل آئٹمز", sectionReferences:"حصہ حوالہ جات", fontNote:"عربی فونٹ صارف منتخب کر سکتا ہے؛ اردو کے لیے Noto Nastaliq Urdu استعمال ہوتا ہے۔", reviewBadge:"مواد جائزہ: علمی تصدیق باقی ہے",
      ...
      fontCleanNaskh:"صاف نسخ", fontTraditionalNaskh:"روایتی نسخ", fontClassicArabic:"کلاسیکی عربی", fontSimpleArabic:"سادہ عربی",
      theme_elder_dark:"بزرگ آسانی — ڈارک", theme_elder_light:"بزرگ آسانی — لائٹ", theme_dark_ambient:"ڈارک ایمبیئنٹ", theme_gold_navy:"سنہرا اور نیوی", theme_haram_light:"حرم لائٹ", theme_green_classic:"سبز کلاسک", theme_sepia:"سیپیا",
  ...
  I18N.ar = Object.assign({}, I18N.en, {
    ...
    arabicTextSize:"حجم النص العربي", arabicFont:"الخط العربي", arabicFontDesc:"اختر خط القراءة العربية.", bismillahSize:"حجم البسملة", bismillahColor:"لون البسملة", smartSentenceFlow:"تدفق الجمل الذكي", waqfPauseSigns:"علامات الوقف والتوقف", showTranslitShort:"النطق اللاتيني", showTranslationShort:"الترجمة",
    theme:"النمط", optLight:"فاتح", optDarkAmbient:"داكن هادئ", optSepia:"سيبيا", scriptUthmani:"عثماني", scriptIndopak:"هند باك", 
  ...
  function rawLocalizedTitle(it) {
    if (!it) return "";
    if (state.settings.lang === "ur") return it.title_ur || TITLE_UR[it.title] || it.title_en || it.title || it.traditional_name || "";
    if (state.settings.lang === "ar") return it.title_ar || it.title_en || it.title || it.traditional_name || "";
    return it.title_en || it.title || it.traditional_name || "";
  }
  function titleLooksTechnical(title) {
    title = String(title || "").trim();
    if (!title) return true;
    if (/(?:^|\b)(?:legacy|hajj_umrah|dua_|azkar_|zikr_|dhikr_|section_|content_|item_)/i.test(title)) return true;
    if (/[_][a-z0-9]+/i.test(title) && !/\s/.test(title)) return true;
    if (/^[a-z0-9_\-]{24,}$/i.test(title)) return true;
    if (/[a-f0-9]{12,}/i.test(title)) return true;
    return false;
  }
  function safeDisplayTitle(it, rawTitle) {
    var candidates = [rawTitle];
    if (it) {
      candidates.push(it.title_en, it.title, it.traditional_name);
      if (it.sectionRefs && it.sectionRefs[0]) candidates.push(it.sectionRefs[0].title);
    }
    for (var i = 0; i < candidates.length; i++) {
      var title = String(candidates[i] || "").replace(/[_\-]{2,}/g, " ").replace(/\s+/g, " ").trim();
      var generic = /^(dua|du'a|duas|azkar|adhkar|zikr|dhikr|supplication|prayer|general)$/i.test(title);
      if (title && !generic && !titleLooksTechnical(title) && title.length <= (IS_TV ? 64 : 92)) return title;
    }
    var cat = it && (it.category || it.main_category || it.type);
    return cat ? localizeCategory(cat) : "Dua";
  }
  ...
  function load() {
    ...
    try {
      ...
      if (!Object.prototype.hasOwnProperty.call(raw, "arabicFont")) {
        var storedArabicFont = localStorage.getItem(LS_AR_FONT);
        if (storedArabicFont) raw.arabicFont = storedArabicFont;
      }
      var s = normalizeSettings(raw);
      if (IS_TV) {
        var rawAr = Number(raw.arScale);
        var needsVisualReset = raw.tvVisualRevision !== "J-tv-closure-premium-leanback-architecture";
        // Rev I-36: keep valid saved user size, but default optional reading layers OFF so the TV starts clean.
        if (!isFinite(rawAr) || rawAr > 2.0 || rawAr < 0.1) { s.arScale = 0.7; repaired = true; }
        ...
        if (s.theme !== "dark-ambient" && s.theme !== "elder-light") { s.theme = "dark-ambient"; repaired = true; }
        s.smartSentenceFlow = true;
        s.tvVisualRevision = "J-tv-closure-premium-leanback-architecture";
      ...
      if (!("showTitle" in raw)) { s.showTitle = DEFAULTS.showTitle; repaired = true; }
      if (!("showArabic" in raw)) { s.showArabic = true; repaired = true; }
      if (!("showPauseMarks" in raw)) { s.showPauseMarks = true; repaired = true; }
      if (IS_TV && raw.tvVisualRevision !== "J-tv-closure-premium-leanback-architecture") {
        s.showTranslit = false;
        s.showEnglish = false;
        s.showUrdu = false;
        s.showTitle = true;
      ...
      applyArabicFontSelection(s);
      if (repaired) { localStorage.setItem(LS, JSON.stringify(s)); localStorage.setItem(LS_AR_FONT, s.arabicFont); }
      return s;
    } catch (e) {
      var safe = normalizeSettings(DEFAULTS);
      if (IS_TV) { safe.arScale = 0.7; safe.bismillahSize = "small"; safe.interval = 30; safe.autoRotate = true; safe.theme = "dark-ambient"; safe.smartSentenceFlow = true; safe.showTranslit = false; safe.showEnglish = false; safe.showUrdu = false; safe.showTranslation = false; safe.showTitle = true; safe.tvVisualRevision = "J-tv-closure-premium-leanback-architecture"; }
      applyArabicFontSelection(safe);
      try { localStorage.setItem(LS, JSON.stringify(safe)); } catch (e2) {}
      return safe;
    }
  }
  function save() { try { state.settings = normalizeSettings(state.settings); applyArabicFontSelection(state.settings); localStorage.setItem(LS, JSON.stringify(state.settings)); localStorage.setItem(LS_AR_FONT, state.settings.arabicFont); } catch (e) {} }
  function savePos() {
    try {
      var sc = $("#readerScroll");
  ...
  function boot() {
    ...
    Promise.all([
      ...
      indexContent();
      applyArabicFontSelection(state.settings);
      applyBodyFlags();
  ...
  function applyBodyFlags() {
    ...
    document.body.setAttribute("data-arabic-font", normalizeArabicFont(state.settings.arabicFont));
    var arQuickValue = $("#arSizeValue");
    if (arQuickValue) arQuickValue.textContent = Math.round((state.settings.arScale || DEFAULTS.arScale) * 100) + "%";
    applyArabicFontSelection(state.settings);
  }

  function applyTheme(id) {
    id = (id === "elder-light") ? "elder-light" : "dark-ambient";
  ...
  function mountTvPrayerRibbon() {
    ...
    if (top && ribbon && ribbon.parentElement !== top) {
      top.insertBefore(ribbon, arSize || $("#themeQuickBtn") || $("#openSettings") || null);
    }
  }
  ...
  function buildViewList() {
    ...
    mix.className = "sec-item primary"; mix.setAttribute("data-view", "mixed"); mix.setAttribute("aria-label", t("mixedFlow") + ": " + t("mixedSub"));
    ...
    CATS.forEach(function (c) {
      ...
      b.className = "sec-item"; b.setAttribute("data-view", "category:" + c); b.setAttribute("aria-label", localizeCategory(c) + ": " + n + " items");
    ...
    (state.sections.sections || []).forEach(function (s) {
      if (!s.count) return;
      var b = document.createElement("button");
      b.className = "sec-item"; b.setAttribute("data-view", "section:" + s.key); b.setAttribute("aria-label", localizeSectionMeta(s) + ": " + s.count + " items");
  ...
  // Rev P Addition: Arabic sentence-ending display lock. Source Arabic remains
  // preserved in content.json; generated display fields are used only where the
  // item has been reviewed as safe for Arabic/Urdu full stop rendering.
  var ARABIC_FULL_STOP = "۔";
  var ARABIC_ACCEPTED_END_RE = /(?:[۔۝۩۞؞؟!?؛]|[\)\]\}﴿﴾]|(?:قلى|صلى|لا|[مجطزصقسع]))\s*$/;
  function displayArabicPlain(it) {
    if (!it) return "";
    if (it.arabic_punctuated && !isQuranicItem(it)) return it.arabic_punctuated;
    return it.arabic || "";
  }
  function displayTajweedHtml(it) {
    if (!it) return "";
    if (it.tajweed_html_punctuated && !isQuranicItem(it)) return it.tajweed_html_punctuated;
    return it.tajweed_html || "";
  }
  function hasAcceptedArabicEnding(text) {
    return ARABIC_ACCEPTED_END_RE.test(String(text || "").replace(/[\u200e\u200f]+/g, "").trim());
  }
...
function autoSize(mode, scale) {
  ...
  function tvShouldStartPopout(it) {
    ...
    var ar = cleanArabicParagraph(stripHtmlForLength(displayArabicPlain(it) || ""));
  ...
  function tvFitMinimums(mode) {
    var userScale = Number(state && state.settings && state.settings.arScale);
    var userMin = (isFinite(userScale) && userScale < 0.7) ? 12 : (({ short: 52, normal: 46, long: 34, very_long: 30 })[mode] || 42);
    return {
      ar: userMin,
      tl: 18,
      tr: 18,
      lhMin: mode === "very_long" ? 1.36 : (mode === "long" ? 1.40 : 1.46),
      gapMin: mode === "very_long" ? 0.54 : (mode === "long" ? 0.60 : 0.72)
    };
  }
  ...
  function render() {
    ...
    arEl.style.fontFamily = arabicFontFamily(s.arabicFont);
    applyArabicFontSelection(s);
    ...
    if (s.showArabic === false) { arEl.innerHTML = ""; arEl.classList.add("hidden"); arEl.classList.remove("tajweed-active"); }
      ...
      if (useTajweed && displayTajweedHtml(it)) arEl.innerHTML = displayTajweedHtml(it);
      else if (useTajweed) arEl.innerHTML = renderTajweedFallback(displayArabicPlain(it));
      else if (isQuranicItem(it)) arEl.innerHTML = renderArabicWithWaqf(displayArabicPlain(it), s.showPauseMarks !== false);
      else arEl.innerHTML = renderArabicReading(displayArabicPlain(it));
    }
    ...
    var runTvFit = function () {
      var signature = tvFitSignature(it, s, reader);
      if (shouldRunFullTvFit(reader, signature)) fitContent(arEl, ar, tl0, tr0);
      else updateTvScrollState(sc0);
  ...
  function tvFitSignature(it, settings, reader) {
    if (!IS_TV || !it) return "non-tv";
    settings = normalizeSettings(settings || state.settings);
    var arabicLen = stripHtmlForLength(displayArabicPlain(it) || "").length;
    var sideLen = (settings.showTranslit !== false ? stripHtmlForLength(it.transliteration || "").length : 0) +
      (settings.showEnglish !== false ? stripHtmlForLength(it.translation || "").length : 0) +
      (settings.showUrdu !== false ? stripHtmlForLength(it.translation_ur || "").length : 0) +
      (settings.showSource ? stripHtmlForLength(it.source || "").length : 0);
    return [
      reader ? (reader.getAttribute("data-size") || it.size_mode || "normal") : (it.size_mode || "normal"),
      document.body.classList.contains("tv-reading-popout") ? "popout" : "card",
      tvFitLengthBucket(arabicLen),
      tvFitLengthBucket(sideLen),
      settings.tvFit !== false ? "fit" : "scroll",
      settings.showTranslit !== false ? "tl1" : "tl0",
      settings.showEnglish !== false ? "en1" : "en0",
      settings.showUrdu !== false ? "ur1" : "ur0",
      settings.showSource ? "src1" : "src0",
      settings.showPauseMarks !== false ? "waqf1" : "waqf0",
      settings.tajweed ? "taj1" : "taj0",
      normalizeArabicFont(settings.arabicFont),
      settings.arScale,
      settings.tlScale,
      settings.trScale
    ].join("|");
  ...
  function shouldRunFullTvFit(reader, signature) {
    if (!IS_TV || !reader || !signature) return true;
    if (state.tvLastFitSignature !== signature || reader.dataset.tvLastFitSignature !== signature) {
      state.tvLastFitSignature = signature;
      reader.dataset.tvLastFitSignature = signature;
      return true;
    }
    return false;
  }

  function markTvFitDirty() {
    state.tvLastFitSignature = "";
    var reader = $("#reader");
    if (reader) reader.dataset.tvLastFitSignature = "";
  }
...
function fitContent(arEl, arStart, tlStart, trStart) {
  ...
  function buildShareText(it) {
    ...
    if (displayArabicPlain(it)) parts.push(displayArabicPlain(it));
  ...
  function shareCardPng(it, cb) {
    try {
      ...
      addBlock(stripWaqfForDisplay(displayArabicPlain(it) || ""), "700 56px serif", 82, fg, "center", true, 40, 999);
  ...
  function bindGlobal() {
    ...
    function stepArabic(d) {
      ...
      if (IS_TV) state.settings.tvVisualRevision = "J-tv-closure-premium-leanback-architecture";
    ...
    var themeQuick = $("#themeQuickBtn");
    if (themeQuick) themeQuick.addEventListener("click", function () {
      ensureSettings();
      var nextTheme = state.settings.theme === "elder-light" ? "dark-ambient" : "elder-light";
      state.settings.theme = nextTheme;
      if (state.draft) state.draft.theme = nextTheme;
  ...
  function zoneEls(z) {
    var list;
    if (z === 0) list = [$("#viewPick"), $("#arDec"), $("#arInc"), $("#themeQuickBtn"), $("#openSettings")].filter(function (el) { return el && el.offsetParent !== null; });
  ...
  function tvSettingEntryFromRow(row) {
    ...
    var dropSelect = row.querySelector(".tv-drop-select");
    ...
    if (stepper) {
      ...
      kind = "dropdown";
      controls = [dropSelect].filter(tvVisible);
    } else if (toggle) {
      kind = "toggle";
  ...
  function tvStepSetting(key, dir, step, min, max) {
    ...
    min = Number(min); if (!isFinite(min)) min = key === "arScale" ? 0.1 : 0.7;
    max = Number(max); if (!isFinite(max)) max = key === "arScale" ? 2.0 : 1.9;
    var draft = ensureDraft();
  ...
  function tvMainArabicStep(dir) {
    ...
    var v = tvClampNumber(current + (dir * 0.1), 0.1, 2.0, 0.7);
    state.settings.arScale = v;
    state.settings.tvVisualRevision = "J-tv-closure-premium-leanback-architecture";
    if (state.draft) state.draft.arScale = v;
    applyArabicFontSelection(state.settings);
    save();
    applyBodyFlags();
  ...
  function tvCycleThemeDirect() {
    if (!IS_TV) return false;
    ensureSettings();
    var next = state.settings.theme === "elder-light" ? "dark-ambient" : "elder-light";
    state.settings.theme = next;
    state.settings.tvVisualRevision = "J-tv-closure-premium-leanback-architecture";
    if (state.draft) state.draft.theme = next;
    save();
    applyTheme(next);
    applyBodyFlags();
    applyTvLivePreviewValues(state.settings);
    if ($("#settingsSheet.open")) syncSettingsUI();
    tvToast(next === "elder-light" ? "Light theme" : "Dark theme");
    return true;
  }
  ...
  function tvDirectControl(el, dir) {
    ...
    if (el.id === "themeQuickBtn") return tvCycleThemeDirect();
    ...
    if (el.classList && el.classList.contains("tv-drop-select")) { return openTvDropdown(el); }
  ...
  function tvSettingsActivate(entry, dir) {
    ...
    if (entry.kind === "dropdown") {
      return openTvDropdown(controls[0]);
    }

    if (entry.kind === "fontselect") {
      return openTvFontSelector(controls[0]);
    }
  ...
  function tvSettingsKey(key) {
    ...
    if ($(".tv-drop-menu.open")) return tvDropdownKey(key);
  ...
  function sheetFocusables() {
    ...
    return Array.prototype.slice.call(sh.querySelectorAll("button, select, input[type=checkbox], input[type=radio]"))
      .filter(function (el) {
        ...
        var host = el.closest(".tv-set-row, .tv-seg, .tv-stepper, .set-row, .settings-cat-chip, .sec-item, .seg, .sheet-foot, .sheet-head, .set-group") || el;
  ...
  function sheetRing(el) {
    ...
    var row = el.closest(".tv-set-row, .tv-seg button, .tv-stepper button, .set-row, .settings-cat-chip, .sec-item, .seg button, .sheet-foot button, .sheet-head button");
  ...
  function renderSearch(q) {
    ...
    matches.forEach(function (it) {
      ...
      b.setAttribute("aria-label", localizeTitle(it) + ": " + localizeCategory(it.category));
  ...
  function buildSettings() {
    ...
    var reading = group("Reading");
    reading.appendChild(toggleRow("Show item name", "Display the dua or zikr name above the Arabic text.", "showTitle"));
    reading.appendChild(toggleRow("Transliteration", "Show or hide Latin transliteration.", "showTranslit"));
    reading.appendChild(toggleRow("English translation", "Show or hide English translation.", "showEnglish"));
    reading.appendChild(toggleRow("Urdu translation", "Show or hide Urdu translation.", "showUrdu"));
    reading.appendChild(toggleRow("Source / reference", "Show or hide the reference line.", "showSource"));
    body.appendChild(reading);

    var arabic = group("Arabic Display");
    arabic.appendChild(selectRow("Arabic Font", "Clean Naskh is the default. Font applies instantly to the reader and pop-out.", "arabicFont", localizedArabicFontOptions()));
    arabic.appendChild(stepperRow("Arabic text size", "Range 10% to 200%, step 10%.", "arScale", 0.1, 0.1, 2.0));
    arabic.appendChild(selectRow("Bismillah size", "Choose refined Bismillah display size.", "bismillahSize", [["Small", "small"], ["Medium", "medium"], ["Large", "large"], ["XL", "xl"]]));
    arabic.appendChild(toggleRow("Pause marks", "Show Quranic waqf and pause marks where available.", "showPauseMarks"));
    arabic.appendChild(toggleRow("Tajweed", "Apply available Tajweed colour coding.", "tajweed"));
    arabic.appendChild(selectRow("Arabic script", "Choose Uthmani or Indo-Pak display style.", "arabicScript", [["Uthmani", "uthmani"], ["Indo-Pak", "indopak"]]));
    body.appendChild(arabic);

    var prayer = group("Prayer & Location");
    prayer.appendChild(toggleRow("Prayer ribbon", "Show or hide the prayer ribbon/header.", "showRibbon"));
    prayer.appendChild(selectRow("Prayer detail level", "Choose how much prayer data is shown.", "prayerHeaderDetail", [["Off", "off"], ["Minimal", "minimal"], ["Standard", "standard"], ["Full", "full"]]));
    prayer.appendChild(selectRow("City / Location", "Manual city override for prayer time fallback.", "city", [["Auto", "auto"], ["Riyadh", "riyadh"], ["Jeddah", "jeddah"], ["Makkah", "makkah"], ["Madinah", "madinah"], ["Dammam", "dammam"]]));
    prayer.appendChild(toggleRow("Auto rotation", "Automatically move to the next item.", "autoRotate"));
    prayer.appendChild(selectRow("Display duration", "Auto-rotation timing per card.", "interval", [["15 sec", 15], ["30 sec", 30], ["45 sec", 45], ["60 sec", 60]]));
    body.appendChild(prayer);

    var appearance = group("Theme & Appearance");
    appearance.appendChild(selectRow("Theme", "Dropdown selector with Dark and Light only.", "theme", [["Dark", "dark-ambient"], ["Light", "elder-light"]]));
    appearance.appendChild(toggleRow("Page progress indicator", "Show reading progress and item counter.", "showProgress"));
    appearance.appendChild(toggleRow("Show tags on display", "Show or hide category tags on the reading card.", "showTags"));
    appearance.appendChild(selectRow("Language", "Settings language.", "lang", [["English", "en"], ["Arabic", "ar"], ["Urdu", "ur"]]));
    body.appendChild(appearance);

    var about = group("App Information");
    var aboutBtn = document.createElement("button"); aboutBtn.type = "button"; aboutBtn.className = "btn-about"; aboutBtn.textContent = t("openAbout") || "About";
  ...
  function buildTvSettings() {
    ...
    body.appendChild(tvSectionHeader("Arabic Display"));
    body.appendChild(tvStepperRow("Arabic text size", "A− / A+ changes Arabic size live from 10% to 200%.", "arScale", 0.1, 0.1, 2.0, true));
    body.appendChild(tvDropdownRow("Arabic Font", "Choose Clean Naskh, Traditional Naskh, Classic Arabic, or Simple Arabic.", "arabicFont", localizedArabicFontOptions(), "ف"));
    body.appendChild(tvSelectRow("Bismillah size", "Choose the display size for Bismillah.", "bismillahSize", [["Small", "small"], ["Med", "medium"], ["Large", "large"], ["XL", "xl"]], "بسم"));
    body.appendChild(tvSelectRow("Arabic script", "Choose Uthmani or Indo-Pak Arabic style for TV reading.", "arabicScript", [["Uthmani", "uthmani"], ["Indo-Pak", "indopak"]], "ش"));
    ...
    body.appendChild(tvSectionHeader("Theme & Appearance"));
    body.appendChild(tvSelectRow("Theme", "Dropdown selector with Dark and Light only.", "theme", [["Dark", "dark-ambient"], ["Light", "elder-light"]], "◑"));
    ...
    body.appendChild(tvSectionHeader("Prayer & Location"));
    body.appendChild(tvToggleRow("Prayer ribbon", "Show or hide the merged next-prayer header immediately.", "showRibbon", "▰"));
    body.appendChild(tvSelectRow("Prayer header detail", "TV-only control for how much Salah/status data appears in the top header.", "prayerHeaderDetail", [["Off", "off"], ["Min", "minimal"], ["Std", "standard"], ["Full", "full"]], "☾"));
    body.appendChild(tvSelectRow("City / Location", "Manual city override for prayer times.", "city", [["Auto", "auto"], ["Riyadh", "riyadh"], ["Jeddah", "jeddah"], ["Makkah", "makkah"], ["Madinah", "madinah"], ["Dammam", "dammam"]], "⌖"));
    body.appendChild(tvSelectRow("Display duration", "Auto-rotation timing per card.", "interval", [["15 sec", 15], ["30 sec", 30], ["45 sec", 45], ["60 sec", 60]], "⏱"));
    ...
    body.appendChild(tvSectionHeader("App Information"));
    body.appendChild(tvInfoRow("About", "Version, content and font notes are available from the About page.", "Open from mobile About button", "i"));
  ...
  function tvRowShell(name, desc, iconText) {
    var r = document.createElement("div"); r.className = "tv-set-row"; r.setAttribute("role", "group"); r.setAttribute("aria-label", name + (desc ? ". " + desc : ""));
    var ic = document.createElement("div"); ic.className = "tv-set-icon"; ic.textContent = iconText || "•"; ic.setAttribute("aria-hidden", "true");
...
function applyTvLivePreviewValues(model) {
  ...
  if (reader && it) {
    ...
    if (arEl) {
      ...
      arEl.style.fontFamily = arabicFontFamily(model.arabicFont);
      applyArabicFontSelection(model);
    }
  ...
  if (IS_TV) {
    requestAnimationFrame(function () {
      ...
      if (arEl && it2 && reader2) {
        var signature = tvFitSignature(it2, model, reader2);
        if (shouldRunFullTvFit(reader2, signature)) fitContent(arEl, autoSize(it2.size_mode, model.arScale),
          Math.round(((it2.size_mode === "short" ? 18 : 16) + 4) * model.tlScale),
          Math.round(((it2.size_mode === "short" ? 19 : 17) + 4) * model.trScale));
        else updateTvScrollState($("#readerScroll"));
      }
    });
  }
}
...
function commitTvDraftLive(rebuildView, renderView) {
  ...
  state.draft.tvVisualRevision = "J-tv-closure-premium-leanback-architecture";
  ...
  function localizedArabicFontOptions() {
    return [
      [t("fontCleanNaskh") || "Clean Naskh", "noto-naskh"],
      [t("fontTraditionalNaskh") || "Traditional Naskh", "scheherazade"],
      [t("fontClassicArabic") || "Classic Arabic", "amiri"],
      [t("fontSimpleArabic") || "Simple Arabic", "noto-sans-arabic"]
    ];
  }

  function tvSelectRow(name, desc, key, opts, iconText) {
    return tvDropdownRow(name, desc, key, opts, iconText);
  }


  function tvDropdownRow(name, desc, key, opts, iconText) {
    var sh = tvRowShell(name, desc, iconText);
    var wrap = document.createElement("div");
    wrap.className = "tv-drop-wrap";
    var btn = document.createElement("button");
    btn.type = "button";
    btn.className = "tv-drop-select";
    btn.setAttribute("data-tv-drop-select", key);
    btn.setAttribute("aria-haspopup", "listbox");
    btn.setAttribute("aria-expanded", "false");
    btn._tvOptions = (opts || []).slice();
    btn.innerHTML = '<span class="tv-drop-current"></span><span class="tv-drop-caret">▾</span>';
    btn.addEventListener("click", function () { openTvDropdown(btn); });
    wrap.appendChild(btn);
    sh.control.appendChild(wrap);
    return sh.row;
  }

  function optionLabelForValue(opts, value, key) {
    for (var i = 0; i < (opts || []).length; i++) if (String(opts[i][1]) === String(value)) return opts[i][0];
    if (key === "city") return cityLabel(value || "auto");
    if (key === "arabicFont") return arabicFontLabel(value);
    return value == null ? "—" : String(value);
  }

  function closeTvDropdown() {
    var menu = $(".tv-drop-menu.open");
    if (menu && menu.parentElement) menu.parentElement.removeChild(menu);
    state.tvDropdown = null;
    $$(".tv-drop-select[aria-expanded='true']").forEach(function (b) { b.setAttribute("aria-expanded", "false"); });
  }

  function tvDropdownButton() {
    return $(".tv-drop-select[aria-expanded='true']") || $(".tv-control-focus.tv-drop-select") || $("#settingsSheet .tv-drop-select");
  }

  function openTvDropdown(btn) {
    if (!IS_TV || !btn) return false;
    closeTvDropdown();
    var key = btn.getAttribute("data-tv-drop-select");
    var opts = btn._tvOptions || [];
    var draft = ensureDraft();
    var current = draft[key];
    var idx = 0;
    opts.forEach(function (o, i) { if (String(o[1]) === String(current)) idx = i; });
    var menu = document.createElement("div");
    menu.className = "tv-drop-menu open";
    menu.setAttribute("role", "listbox");
    menu.setAttribute("aria-label", (key || "Setting") + " options");
    opts.forEach(function (o, i) {
      var item = document.createElement("button");
      item.type = "button";
      item.className = "tv-drop-option";
      item.setAttribute("role", "option");
      item.setAttribute("data-drop-val", JSON.stringify(o[1]));
      item.setAttribute("aria-selected", i === idx ? "true" : "false");
      item.textContent = o[0];
      item.addEventListener("click", function () { selectTvDropdownValue(o[1], true); });
      menu.appendChild(item);
    });
    btn.parentElement.appendChild(menu);
    btn.setAttribute("aria-expanded", "true");
    state.tvDropdown = { key:key, index:idx, options:opts };
    focusTvDropdownOption(idx);
    return true;
  }

  function focusTvDropdownOption(index) {
    var opts = $$(".tv-drop-menu.open .tv-drop-option");
    if (!opts.length) return false;
    index = Math.max(0, Math.min(index == null ? 0 : index, opts.length - 1));
    state.tvDropdown = state.tvDropdown || { key:"theme", index:index };
    state.tvDropdown.index = index;
    opts.forEach(function (b, i) {
      var active = i === index;
      b.classList.toggle("tv-drop-option-focus", active);
      b.setAttribute("aria-current", active ? "true" : "false");
      if (active) { try { b.focus({ preventScroll: true }); } catch (e) { try { b.focus(); } catch (e2) {} } }
    });
    return true;
  }

  function selectTvDropdownValue(value, closeAfter) {
    var key = (state.tvDropdown && state.tvDropdown.key) || "theme";
    if (key === "arabicFont") value = normalizeArabicFont(value);
    var selectedLabel = optionLabelForValue(state.tvDropdown && state.tvDropdown.options, value, key);
    tvSetDraftValue(key, value, 0);
    if (key === "arabicFont") applyArabicFontSelection(state.settings);
    if (closeAfter !== false) closeTvDropdown();
    syncSettingsUI();
    refreshTvSettingsFocus();
    tvToast((key === "theme" ? "Theme" : key === "interval" ? "Display duration" : key === "city" ? "City" : "Setting") + ": " + selectedLabel);
    return true;
  }

  function tvDropdownKey(key) {
    var menu = $(".tv-drop-menu.open");
    if (!menu) return false;
    var opts = $$(".tv-drop-menu.open .tv-drop-option");
    if (!opts.length) { closeTvDropdown(); return true; }
    var idx = state.tvDropdown && state.tvDropdown.index != null ? state.tvDropdown.index : 0;
    if (key === "ArrowDown") { focusTvDropdownOption(Math.min(idx + 1, opts.length - 1)); return true; }
    if (key === "ArrowUp") { focusTvDropdownOption(Math.max(idx - 1, 0)); return true; }
    if (key === "Enter" || key === " ") {
      var raw = opts[idx].getAttribute("data-drop-val"), val;
      try { val = JSON.parse(raw); } catch (e) { val = raw; }
      return selectTvDropdownValue(val, true);
    }
    if (key === "Escape" || key === "Settings") { closeTvDropdown(); var b = tvDropdownButton(); if (b) { try { b.focus({ preventScroll:true }); } catch (e) {} } return true; }
    return true;
  }
  ...
  opts.forEach(function (o) {
    ...
    b.setAttribute("aria-label", name + ": " + o[0]);
  ...
  function stepperRow(name, desc, key, step, min, max) {
    ...
    step = Number(step); if (!isFinite(step) || step <= 0) step = 0.1;
    min = Number(min); if (!isFinite(min)) min = key === "arScale" ? 0.1 : 0.7;
    max = Number(max); if (!isFinite(max)) max = key === "arScale" ? 2.0 : 2.0;
    function clamp(v) { return Math.min(max, Math.max(min, Math.round(v * 100) / 100)); }
    minus.addEventListener("click", function () { var draft = ensureDraft(); draft[key] = clamp(Number(draft[key]) - step); val.textContent = pct(draft[key]); previewFonts(); });
    plus.addEventListener("click", function () { var draft = ensureDraft(); draft[key] = clamp(Number(draft[key]) + step); val.textContent = pct(draft[key]); previewFonts(); });
  ...
  function selectRow(name, desc, key, opts) {
    ...
    sel._settingsOptions = (opts || []).slice();
    opts.forEach(function (o) {
      var op = document.createElement("option"); op.value = String(o[1]); op.textContent = o[0]; sel.appendChild(op);
    });
    sel.value = String((state.draft && state.draft[key]) || (state.settings && state.settings[key]) || (opts[0] && opts[0][1]));
    sel.addEventListener("change", function () {
      var draft = ensureDraft(), val = sel.value;
      (sel._settingsOptions || []).forEach(function (o) { if (String(o[1]) === sel.value) val = o[1]; });
      draft[key] = val;
      if (key === "arabicFont") applyArabicFontSelection(draft);
      if (key === "theme") applyTheme(draft.theme);
      if (key === "lang") { var savedLang = state.settings.lang; state.settings.lang = draft.lang; applyLang(); state.settings.lang = savedLang; }
      previewFonts();
    });
  ...
  function syncSettingsUI() {
    ...
    $$("select[data-key]").forEach(function (s) { s.value = String(model[s.getAttribute("data-key")]); });
    ...
    $$(".tv-toggle[data-tv-toggle]").forEach(function (b) {
      ...
      b.setAttribute("aria-label", (b.closest(".tv-set-row")?.querySelector(".tv-set-name")?.textContent || key) + ": " + (on ? "On" : "Off"));
    });
    ...
    $$(".tv-drop-select[data-tv-drop-select]").forEach(function (btn) {
      var key = btn.getAttribute("data-tv-drop-select"), val = model[key];
      if (key === "arabicFont") val = normalizeArabicFont(val);
      var cur = btn.querySelector(".tv-drop-current");
      var label = optionLabelForValue(btn._tvOptions || [], val, key);
      if (cur) cur.textContent = label;
      btn.setAttribute("aria-label", (btn.closest(".tv-set-row")?.querySelector(".tv-set-name")?.textContent || key) + ": " + label);
    });
  ...
  function markThemes() {
    // Theme cards were removed in Rev P. Theme state is now controlled only by dropdown selectors.
  }
  ...
  function applySettings() {
    ...
    if (IS_TV) state.draft.tvVisualRevision = "J-tv-closure-premium-leanback-architecture";
    state.settings = Object.assign({}, state.draft);
    markTvFitDirty();
    applyArabicFontSelection(state.settings);
    save();
  ...
  function azkarProbeFindItem(query) {
    ...
    for (var i = 0; i < pool.length; i++) {
      ...
      seen[key] = true;
      var hay = normToken([it.display_id, it.id, it.title, it.title_ar, it.title_ur, it.category, it.type, it.source, displayArabicPlain(it), it.transliteration, it.translation, it.translation_ur].join(" "));
  ...
  window.__azkar = { AR_FONTS: AR_FONTS, arabicFontFamily: arabicFontFamily, parseRepeat: parseRepeat, buildMixed: buildMixed, CATS: CATS, getSectionRefs: getSectionRefs, stripWaqfForDisplay: stripWaqfForDisplay, renderArabicWithWaqf: renderArabicWithWaqf, renderTajweedFallback: renderTajweedFallback, displayArabicPlain: displayArabicPlain, displayTajweedHtml: displayTajweedHtml, hasAcceptedArabicEnding: hasAcceptedArabicEnding, buildShareText: buildShareText, tvDirectControl: tvDirectControl, tvMainKey: tvMainKey, getState: function(){ return state; }, initTvSettingsFocus: initTvSettingsFocus, forceTvSettingsInitialFocus: forceTvSettingsInitialFocus };
