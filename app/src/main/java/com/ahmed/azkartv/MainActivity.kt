 ...
 *
 * Rev J architecture note: any future Compose for TV or Leanback home/browse
 * layer must remain a launcher/browse layer only. The WebView reader stays the
 * source of truth for reading, settings, pop-out mode, rotation and remote keys.
 */
class MainActivity : AppCompatActivity() {
    ...
    /**
     * Maps Android TV / Google TV D-pad and remote key codes into the small
     * JavaScript key vocabulary consumed by the existing WebView reader.
     */
    ...
    /**
     * Sends a mapped TV remote key into the bundled JavaScript bridge.
     * Behaviour intentionally remains delegated to the web layer to preserve
     * the already-working reader, settings, pop-out and pause/resume controls.
     */
    ...
    /**
     * Handles ACTION_DOWN TV remote events and prevents duplicate dispatch into
     * WebView once JavaScript accepts the remote key.
     */
    ...
    /**
     * Primary hardware-key dispatch path. TV keys are bridged to JavaScript;
     * non-TV and non-back keys continue through WebView and Android defaults.
     */
